...
export default function AccountEditor({userInfo} : {userInfo: UserInfo}) {
    ...
    return <form className="flex w-1/2 flex-col gap-4"> 
                ...
                <Input type="password" label="New Password" name="New password"
                    ...
                    isInvalid={isInvalidPassword} errorMessage={isInvalidPassword && "Please enter at least 8 characters"} />
                <Input type="password" label="Confirm Password" isRequired name="Confirm new password"
                    value={passwordConfirm} onValueChange={setPasswordConfirm}
                    isInvalid={isInvalidConfirm} 
                    errorMessage={isInvalidConfirm && "Your password confirmation does not match your new password"} />
                <Popover color="danger" isOpen={accountMessage !="" } onOpenChange={() => setAccountMessage("")}>
                    <PopoverTrigger>
                        <Button color="danger" onClick={handleAccountSubmit}> Update Password </Button>
                    </PopoverTrigger>
                    <PopoverContent >
